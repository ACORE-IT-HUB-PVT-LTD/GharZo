...
const TenantRentPayments = () => {
  ...
  const [rentHistory, setRentHistory] = useState([
    ...
    },
    {
      ...
      roomId: "Tower A - 401",
      method: "Razorpay",
      paymentId: "pay_def456uvw789",
    },
  ]);

  const [totalDues, setTotalDues] = useState(18300);
  const [duesByType, setDuesByType] = useState({
    Rent: 15000,
    Electricity: 2500,
    Water: 800,
  ...
  const [landlordId, setLandlordId] = useState("LANDLORD123");
  ...
  const fetchAccommodationAndDues = async () => {
    ...
    */
    ...
    // ✅ Dummy data is already set in useState
    setLoading(false);
    console.log("Using dummy data - API calls are disabled");
  };
  ...
  const handleRefreshDues = () => {
    showCustomDialog('info', 'Refreshing with dummy data (API disabled)');
    calculateDues(rentHistory);
  };
  ...
  const handlePayNow = async (billId, amount) => {
    ...
    showCustomDialog('info', `Payment feature is disabled in demo mode.\n\nBill: ${billId}\nAmount: ₹${amount.toLocaleString()}\n\nIn production, this would open Razorpay payment gateway.`);
    
    // Simulate successful payment after 2 seconds
    setTimeout(() => {
      const updatedHistory = rentHistory.map(bill => 
      ...
      setRentHistory(updatedHistory);
      calculateDues(updatedHistory);
      showCustomDialog('success', `Demo payment successful!\n\nBill ${billId} marked as PAID.\nPayment ID: pay_demo_${Date.now()}`);
    }, 2000);

    /*
    // Original Razorpay integration code (commented out)
    try {
      const res = await loadRazorpayScript();
      if (!res) {
        showCustomDialog('error', "Razorpay SDK failed to load. Are you online?");
  ...
  const handleReceiptClick = async (paymentId) => {
    // ✅ Simulated receipt for demo
    const demoReceipt = {
      receiptNo: `RCP-${Date.now()}`,
      date: new Date().toISOString(),
      tenantId: tenantId || "TENANT123",
      property: propertyId || "Sunrise Apartments",
      paymentId: paymentId,
      method: "Razorpay",
      status: "PAID",
      amount: 15000,
      totalPaid: 15000,
      note: "Demo receipt - API disabled",
    };

    setCurrentReceipt(demoReceipt);
    setReceiptNumber(demoReceipt.receiptNo);
    setShowReceipt(true);
  ...
  const handleDownloadPDF = async (paymentId) => {
    showCustomDialog('info', 'PDF download feature is disabled in demo mode.\n\nIn production, this would download the receipt PDF.');
