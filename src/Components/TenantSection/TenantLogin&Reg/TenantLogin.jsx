...
// =====================================================
// HARDCODED DUMMY CREDENTIALS (Testing Purpose)
// =====================================================
const DUMMY_PHONE = "9876543210";
const DUMMY_OTP = "123456";
const DUMMY_TOKEN = "dummy_tenant_token_for_testing";
const DUMMY_ROLE = "tenant";
// =====================================================
...
function TenantLogin({ onClose }) {
  ...
  // =====================================================
  // AUTO-LOGIN ON MOUNT (Testing Purpose)
  // Seedha /tenant pe redirect kar deta hai
  // =====================================================
  useEffect(() => {
    localStorage.setItem("tenanttoken", DUMMY_TOKEN);
    localStorage.setItem("role", DUMMY_ROLE);
    login({ phone: DUMMY_PHONE, role: DUMMY_ROLE, isRegistered: true });
    toast.success("Auto Login Successful! (Testing Mode)");
    navigate("/tenant");
  }, []);
  // =====================================================
  ...
  // =====================================================
  // SEND OTP - API COMMENTED OUT
  // Ab sirf toast dikhaega, koi API call nahi hogi
  // =====================================================
  const handleSendOtp = async (e) => {
    e.preventDefault();
    if (phone.length !== 10) {
    ...
    try {
      ...
      toast.success("OTP sent successfully! (Testing Mode)");
      setOtpSent(true);
      setResendAttempts(0);
      setCountdown(0);
      setVerificationError("");
  ...
  const handleResetOtp = async (e) => {
    ...
    try {
      ...
      toast.success("New OTP sent successfully! (Testing Mode)");
      const waitTimes = [15, 30, 60, 120, 240];
      const nextWait = waitTimes[resendAttempts] || 300;
      setResendAttempts((prev) => prev + 1);
      setCountdown(nextWait);
  ...
  // =====================================================
  // VERIFY OTP - API COMMENTED OUT
  // Dummy OTP (123456) se verify ho jaayega
  // =====================================================
  const handleVerifyOtp = async (e) => {
    e.preventDefault();
    if (otp.length !== 6) {
    ...
    try {
      ...
      // ---- Dummy Response Simulate ----
      // const isRegistered = response.data?.isRegistered;
      // const token = response.data?.token;
      // const userRole = response.data?.user?.role || userType;
      const isRegistered = true;
      const token = DUMMY_TOKEN;
      const userRole = DUMMY_ROLE;
      // ---- End Dummy Response ----
      ...
      login({ phone, role: userRole, isRegistered: true });
      toast.success("Login successful! (Testing Mode)");
      onClose?.();
      navigate("/tenant");
    } catch (error) {
      toast.error(error.response?.data?.message || "Failed to verify OTP");
