...
const OrganizationRoomAdd = () => {
  ...
  const getToken = () => localStorage.getItem("orgToken");
    ...
    try {
      ...
      if (res.data.success) {
        setBedsModalData((prev) =>
          prev.map((b) =>
            b.bedId === bed.bedId
              ? { ...b, price: bed.price, status: bed.status, images: updatedImages }
              : b
          )
        );
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 bg-gray-100 min-h-screen text-black rounded-xl shadow-lg relative">
                ...
                <form onSubmit={handleEditBedSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleEditBedChange("price", e.target.value)
                      }
      ...
      {showBedForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <motion.div
            className="bg-gray-50 p-6 rounded-2xl shadow-lg w-full max-w-lg max-h-[80vh] overflow-y-auto"
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
          >
...
export default OrganizationRoomAdd;