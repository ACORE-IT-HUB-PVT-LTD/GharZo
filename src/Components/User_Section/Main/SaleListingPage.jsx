            ...
            <div className="relative">
              ...
              {activeFilter === "budget" && (
                <div className="absolute top-12 left-0 w-72 bg-white shadow-xl border rounded-lg p-4 z-50">
                  ...
                  {/* Quick presets for sale */}
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {[
                      { label: "< 10L", max: "1000000" },
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 bg-orange-50 border border-orange-200 text-orange-700 rounded-full text-xs font-medium hover:bg-orange-100 transition-all"
                      >
                        {p.label}
                      </button>
                    ))}
...
const SaleListingPage = () => {
  ...
  // Filter states — default "Sale" for this page
  const [searchTerm, setSearchTerm] = useState("");
  const [debouncedSearch, setDebouncedSearch] = useState("");
  const [listingType, setListingType] = useState("Sale"); // ✅ default Sale
  ...
  useEffect(() => {
    ...
    const fetchProperties = async () => {
      ...
      try {
        ...
        // ✅ "Sale" / "Rent" — exact Title Case
        if (listingType) params.append("listingType", listingType);

        // ✅ Exact property type from API — "Flat", "Plot", "Villa" etc.
  ...
  const handleReset = () => {
    setSearchTerm("");
    setListingType("Sale"); // reset back to Sale (not empty)
    setPropertyType("");
    setMinBudget("");
    setMaxBudget("");
    setBhk([]);
  ...
  // Format price for sale (in Lakhs/Crores)
      ...
      <div className="py-8 sm:py-10 px-4 sm:px-6 lg:px-10">
        ...
        {hasActiveFilters && (
          <div className="mb-6 flex flex-wrap gap-2 items-center">
            ...
            {(minBudget || maxBudget) && (
              <span className="inline-flex items-center gap-1 px-3 py-1 bg-[#f15a24] text-white rounded-full text-xs font-medium">
                💰 {formatSalePrice(Number(minBudget) || 0)} – {maxBudget ? formatSalePrice(Number(maxBudget)) : "∞"}
        ...
        {!loading && properties.length === 0 && (
          ...
          >
            ...
            <p className="text-gray-500 mb-6">
              No properties match your criteria. Try adjusting the filters.
            </p>
            <button
              onClick={handleReset}
            ...
            {properties.map((property, index) => (
              ...
              >
                ...
                <div className="relative h-60 overflow-hidden bg-gray-100">
                  ...
                  {/* Price badge — green for sale */}
                  <div className="absolute top-4 left-4 bg-green-600 text-white px-3 py-1.5 rounded-full font-semibold text-sm shadow-lg">
                  ...
                  {property.listingType && (
                    <div
                      className={`absolute top-4 right-4 px-2.5 py-1 rounded-full text-xs font-bold shadow ${
