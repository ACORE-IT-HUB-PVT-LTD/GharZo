...
export default function LandlordComplaints() {
  ...
  const fetchComplaints = async (status = "", pageNum = 1) => {
    ...
    try {
      ...
      // Build URL - get all complaints for landlord
      let url = `https://api.gharzoreality.com/api/complaints/landlord/all-complaints?page=${pageNum}`;
      
      // Add propertyId filter if available
      if (propertyId) {
