...
function Login({ role, onClose }) {
  ...
  // Check for valid token and redirect to dashboard
  useEffect(() => {
    const token = localStorage.getItem("sellertoken");
    if (token) {
      console.log("Token found, redirecting to /seller");
      navigate("/seller");
    }
  }, [navigate]);

  const userType = role?.toLowerCase() || "seller";
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      const response = await axios.post(`${baseurl}api/seller/request-otp`, {
        mobile: phone,
        userType,
      });
      ...
      toast.error(
        error.response?.data?.message ||
          "Failed to send OTP. Please check the API endpoint."
      );
    }
  };
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message ||
          "Failed to resend OTP. Please check the API endpoint."
      );
      console.error("OTP Resend Error:", error.response?.status, error.message);
    } finally {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      const response = await axios.post(`${baseurl}api/seller/verify-otp`, {
        mobile: phone,
        otp,
      });
      ...
      if (success) {
        if (token) {
          ...
          setTimeout(() => {
            if (onClose) onClose();
            navigate("/seller_signup", { state: { mobile: phone } });
          }, 2000);
        }
      } else {
      ...
      toast.error(
        error.response?.data?.message ||
          "Failed to verify OTP. Please check the API endpoint."
      );
    }
  };
        ...
        <div className="w-full lg:w-[45%] bg-gradient-to-b from-[#0c2344] to-[#0b4f91] relative overflow-hidden flex items-center justify-center p-6 sm:p-8">
          ...
          >
            ...
            >
              ...
              <div>
                <h2 className="text-3xl sm:text-4xl font-bold text-white mb-2">
                  {!otpSent ? "Welcome back Seller" : "Verify OTP"}
                </h2>
                <p className="text-slate-400 text-sm">
                  {!otpSent ? (
                    <>
                      New seller?{" "}
                      <button
                        onClick={() => navigate("/seller_signup", { state: { mobile: phone } })}
            ...
            >
              ...
              <button
                onClick={() => {
                  console.log("Manual navigation to /seller_signup with mobile:", phone);
                  navigate("/seller_signup", { state: { mobile: phone } });
                }}
          ...
          <div className="relative h-full flex flex-col justify-end p-12 z-10">
            ...
            >
              ...
              <p className="text-lg text-white/80 leading-relaxed">
                Reach thousands of customers, manage your inventory & scale your sales effortlessly. Join our thriving seller community today.
              </p>
            </motion.div>
